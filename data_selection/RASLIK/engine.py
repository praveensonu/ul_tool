...
from .RapidGrad import RapidGrad
import numpy as np
import time
import json
            ...
            if cal_word_infl < 0:
                ...
                if grad_z_vec is None:
                    ...
                    if config.influence.RapidGrad.enable and isinstance(config.influence.RapidGrad.RapidGrad_K, list):
                        ...
                        for i, (path, k) in enumerate(zip(config.influence.RapidGrad.multi_k_save_path_list, config.influence.RapidGrad.RapidGrad_K)):
                            torch.save(grad_z_vec_list[i], path + f"/train_grad_K{k}_{real_id}.pt")
                ...
                for i in range(len(test_dataset)):
                    ...
                    if config.influence.deepspeed.enable and config.influence.calculate_infl_in_gpu:
                        ...
                        influence = torch.sum(grad_z_vec.float() * s_test_vec_t.float())

                    if config.influence.calculate_infl_in_gpu == True:
                        influence = influence.cpu()
                    influence = influence.numpy()

                    # Add before the NaN check:
                    if torch.isnan(grad_z_vec).any():
                        print(f"NaN in grad_z_vec at idx={idx}")
                    if torch.isnan(s_test_vec_t).any():
                        print(f"NaN in s_test_vec at test={i}")
...
def calc_infl_mp(config):
    ...
    if gpu_num < 1:
        raise RuntimeError("RASLIK gradient caching requires at least one CUDA GPU.")
